...
from lsst.ts.IntegrationTests import AuxTelPointAzEl


class AuxTelPointAzElTestCase(BaseTestClass):
    """
    Test the AuxTel Point AzEl integration test scripts.
    """

    # Use AuxTel ScriptQueue.
    index = 2

    async def test_auxtel_point_azel(self) -> None:
        """Execute the AuxTelPointAzEl integration test script,
        which runs the standard/auxtel/point_azel.py
        script.
        """
        # Instantiate the AuxTelPointAzEl integration tests.
        args = ["0.0", "80.0", "-t", "park"]
        script_class = AuxTelPointAzEl(args)
    ...
    async def test_auxtel_point_azel_flatfield(self) -> None:
        """Execute the AuxTelPointAzEl integration test script,
        which runs the standard/auxtel/point_azel.py
        script.
        ...
        """
        # Instantiate the AuxTelPointAzEl integration tests.
        args = ["188.7", "39.0", "-t", "flatfield", "-i", "atdome,atdometrajectory"]
        script_class = AuxTelPointAzEl(args)
        # Get number of scripts
        num_scripts = len(script_class.scripts)
        print(
    ...
    async def test_auxtel_point_azel_rotator_angle(self) -> None:
        """Execute the AuxTelPointAzEl integration test script,
        which runs the standard/auxtel/point_azel.py
        script.
        Test changing the Rotator angle using the vent position.
        """
        # Instantiate the AuxTelPointAzEl integration tests.
    ...
    async def test_auxtel_point_azel_no_args(self) -> None:
        """Execute the AuxTelPointAzEl integration test script,
        which runs the standard/auxtel/point_azel.py
        script.
